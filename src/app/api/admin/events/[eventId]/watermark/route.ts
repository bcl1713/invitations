...
import { setEventWatermarkImage } from '@/modules/events/event-service';
...
export async function POST(
  ...
  const file = formData.get('watermarkImage');

  if (file instanceof File && file.size > 0) {
    const storedFileName = await saveUploadedImage(file);
    await setEventWatermarkImage(eventId, storedFileName);
  }
